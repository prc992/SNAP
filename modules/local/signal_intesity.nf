process signalIntensityCalculation {
    ...
    if [ "\$(echo '${enrichment_mark}' | tr '[:upper:]' '[:lower:]')" = "medip" ]; then
        Rscript ${chRMEDIPSignalCalculation} ${bed_file} ${chDACFileRef} ${chRegions_of_interest_MEDIP_signal} ${chHousekeeping_MEDIP_signal}
    elif [ "\$(echo '${enrichment_mark}' | tr '[:upper:]' '[:lower:]')" = "h3k4me3" ]; then
        Rscript ${chRMARKSSignalCalculation} ${bed_file} ${chRegions_of_interest_MARKS_signal} ${chHousekeeping_H3K4ME3_signal} ${enrichment_mark}
    elif [ "\$(echo '${enrichment_mark}' | tr '[:upper:]' '[:lower:]')" = "h3k27ac" ]; then
        Rscript ${chRMARKSSignalCalculation} ${bed_file} ${chRegions_of_interest_MARKS_signal} ${chHousekeeping_H3K27AC_signal} ${enrichment_mark}
    fi
