process signalIntensityCalculation {
    ...
    each path(chRMEDIPSignalCalculation), each path(chRMARKSSignalCalculation)
    each path(chRegions_of_interest_MEDIP_signal), each path(chRegions_of_interest_MARKS_signal)
    each path(chHousekeeping_MEDIP_signal), each path(chHousekeeping_H3K4ME3_signal), each path(chHousekeeping_H3K27AC_signal)
